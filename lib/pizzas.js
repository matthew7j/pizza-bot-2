...
const handlePizzaMessage = async (app, message) => {
  let counter = 0;
  let totalPizzas = 0;

  const splitMessage = message.text.split(' ').reverse().filter(el => el !== '');
  let pizzaUsersObjectArray = [];
  ...
  await forEach(splitMessage, async msg => {
    if (msg === ':pizza:') {
      ...
      if (pizzaUsersObjectArray[counter].recipients.length === 0) {
        pizzaUsersObjectArray[counter].pizzas += 8;
      } else {
        pizzaUsersObjectArray.push(createNewEmptyPizzaUserObject());
        pizzaUsersObjectArray[++counter].pizzas += 8;
      }
      ...
      if (msg && msg.trim() && pizzaUsersObjectArray[counter].recipients.length > 0) {
        pizzaUsersObjectArray.push(createNewEmptyPizzaUserObject());
        counter++;
      }
    }
  ...
  pizzaUsersObjectArray = pizzaUsersObjectArray.filter(obj => obj.pizzas > 0 && obj.recipients.length > 0);
